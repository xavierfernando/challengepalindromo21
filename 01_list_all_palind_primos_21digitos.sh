...
listaImpares() {
  ...
  rm -f arr_04_setembro.txt

  horainicial=$(date +%s)
  # echo hora inicial $horainicial
  declare -a arr=()
  i=1
  for ((impar = 3; impar <= 99999999; impar += 2)); do

    if [ ${impar: -1} -ne 5 ]; then
      arr+=($impar)
      # echo $i ${arr[-1]}
      # ((i++))
    fi
  ...
  horafinal=$(date +%s)
  echo "duracao antes de gravar array: $((($horafinal - $horainicial) / 60)) minutos "
  echo "${arr[@]}"
  echo "apos execucao do echo"
  printf '%s\n' "${arr[@]}" 
  echo "apos execucao do printf"
  printf '%s\n' "${arr[@]}" > arr_04_setembro.txt
...
sequenciainicial=100000000000000000000
